...
// Global crop bridge
if(typeof window!=='undefined') window.__shambamap_crop = 'maize'


// ═══ ALL FUNCTIONS FROM app-logic-raw.js ═══
// getDistCoords is imported from data/index.js
...
function simFertilizer(id){var r=REAL[id];if(!r)return null;var s=r.soil;var c=window.__shambamap_crop;
...
function simCredit(id){var d=D.find(function(x){return x.id===id});if(!d)return{score:500};var r=REAL[id];var score=400;var c=window.__shambamap_crop;
score+=d.srr[c]*.8;
score+=(100-d.vrr)*.3;
...
function calcPri(d){var c=window.__shambamap_crop;var gap=100-d.srr[c];var r=REAL[d.id];var dm=d.dr>=2?1.4:1;var sa=1;if(d.sd&&d.sd[0]&&d.sd[0].d.indexOf('No')>=0)sa=1.5;var area=r?(r.maize.a+r.wheat.a+r.beans.a)/100000:2;return Math.round(gap*area*dm*sa)}
...
function renderLegend(){
  ...
  var legendStr='<div class="sh">'+(t[mod]||'')+'</div><div class="bar">'+(b[mod]||b.seed)+'</div>';
  var el=document.getElementById('legend');
  if(el) el.innerHTML=legendStr;
  return legendStr;
}
...
function renderList(){
  var sorted=[].concat(D).sort(function(a,b){
    ...
    return a.srr[window.__shambamap_crop]-b.srr[window.__shambamap_crop];
  });
  var listStr=sorted.map(function(d,i){
    var lbl=getLabel(d);var c2=getCol(d);var act=sel&&sel.id===d.id?'on':'';
    return '<div class="dl-i '+act+'" onclick="window.__shambamap_selDist(\''+d.id+'\')"><span class="rk">'+(i+1)+'</span><span class="dn">'+d.n+'</span><div class="br"><div class="bf" style="width:50%;background:'+c2+'"></div></div><span class="vl" style="color:'+c2+'">'+lbl+'</span></div>';
  }).join('');
  var el=document.getElementById('distList');
  if(el) el.innerHTML=listStr;
  return listStr;
}
...
function renderLayers(){
  var layerStr=LAYERS.map(function(l){
    var isActive=l.mod&&l.mod===mod;
    return '<div class="ly'+(isActive?' on':l.on?' on':'')+'" data-id="'+l.id+'" data-mod="'+(l.mod||'')+'" onclick="window.__shambamap_toggleLayer&&window.__shambamap_toggleLayer(this)"><div class="ck">✓</div><div class="dt" style="background:'+l.color+'"></div><span class="nm">'+l.name+'</span></div>';
  }).join('');
  var el=document.getElementById('layers');
  if(el) el.innerHTML=layerStr;
  return layerStr;
}
...
function renderDetail(){
  if(!sel)return;
  var d=sel,r=REAL[d.id],c=window.__shambamap_crop;
  var cn={maize:'Maize',wheat:'Wheat',beans:'Beans',sorghum:'Sorghum'}[c];
  var html='';
  ...
  if(recs.length>0 && (mod==='seed'||mod==='ksc')){
    ...
    var kCrop=window.__shambamap_crop;
...
document.addEventListener('click',function(e){if(!e.target.closest('.user-pill')){var um=document.getElementById('userMenu');if(um)um.classList.remove('show')}});



// ═══ VOICE BUTTON (Swahili only) ═══
const VOICE_LANGS=[{code:'sw',name:'Kiswahili',flag:'\u{1F1F9}\u{1F1FF}',endpoint:'/api/voice'}]
...
function VoiceButton({externalOpen,setExternalOpen}){
  ...
  const wsRef=useRef(null),ctxRef=useRef(null),procRef=useRef(null),streamRef=useRef(null)
  const playQRef=useRef([]),playingRef=useRef(false),playCtxRef=useRef(null),stopRef=useRef(null)
  const langInfo=VOICE_LANGS.find(l=>l.code===lang)||VOICE_LANGS[0]
  const nextTimeRef=useRef(0)
  const playNext=useCallback(async()=>{
    if(playingRef.current)return;playingRef.current=true;setSpeaking(true)
    const drain=()=>{
      ...
      const ctx=playCtxRef.current,chunks=[];while(playQRef.current.length>0)chunks.push(playQRef.current.shift())
      ...
      const now=ctx.currentTime,startAt=Math.max(now,nextTimeRef.current);nextTimeRef.current=startAt+buf.duration
      src.onended=()=>{if(playQRef.current.length>0)drain();else{playingRef.current=false;setSpeaking(false);nextTimeRef.current=0}}
      src.start(startAt)
    };setTimeout(drain,80)
  },[])
  const stop=useCallback(()=>{
    if(wsRef.current&&wsRef.current.readyState===WebSocket.OPEN){try{wsRef.current.send(JSON.stringify({type:'stop'}))}catch(e){}wsRef.current.close()}
    wsRef.current=null
    ...
    playQRef.current=[];playingRef.current=false;setActive(false);setListening(false);setSpeaking(false)
  },[]);stopRef.current=stop
  const start=useCallback(async()=>{
    setErr('');setInTx('');setOutTx('')
    try{
      const proto=window.location.protocol==='https:'?'wss:':'ws:',host=window.location.host
      const ws=new WebSocket(proto+'//'+host+langInfo.endpoint);wsRef.current=ws
      await new Promise((res,rej)=>{ws.onopen=()=>res();ws.onerror=()=>rej(new Error('Cannot connect to voice server.'));setTimeout(()=>rej(new Error('Timeout')),5000)})
      ws.onmessage=(e)=>{const msg=JSON.parse(e.data);if(msg.type==='audio'){const raw=atob(msg.data);const bytes=new Uint8Array(raw.length);for(let i=0;i<raw.length;i++)bytes[i]=raw.charCodeAt(i);playQRef.current.push(bytes);playNext()}else if(msg.type==='input_transcript')setInTx(p=>p+msg.text);else if(msg.type==='output_transcript')setOutTx(p=>p+msg.text);else if(msg.type==='error'){setErr(msg.message);setTimeout(()=>stopRef.current?.(),2000)}else if(msg.type==='turn_complete'){setSpeaking(false);setTimeout(()=>{setInTx('');setOutTx('')},3000)}}
      ...
      const audioCtx=new AudioContext();ctxRef.current=audioCtx;const nativeSR=audioCtx.sampleRate
      const source=audioCtx.createMediaStreamSource(stream),proc=audioCtx.createScriptProcessor(2048,1,1);procRef.current=proc
      let silenceFrames=0
      proc.onaudioprocess=(ev)=>{if(ws.readyState!==WebSocket.OPEN)return;const f32=ev.inputBuffer.getChannelData(0);let maxAmp=0;for(let i=0;i<f32.length;i++){const a=Math.abs(f32[i]);if(a>maxAmp)maxAmp=a}if(maxAmp<0.01){silenceFrames++;if(silenceFrames>10)return}else{silenceFrames=0}let samples=f32;if(nativeSR!==SAMPLE_RATE){const ratio=nativeSR/SAMPLE_RATE;const nl=Math.round(f32.length/ratio);samples=new Float32Array(nl);for(let i=0;i<nl;i++)samples[i]=f32[Math.round(i*ratio)]}const i16=new Int16Array(samples.length);for(let i=0;i<samples.length;i++)i16[i]=Math.max(-32768,Math.min(32767,Math.round(samples[i]*32768)));ws.send(JSON.stringify({type:'audio',data:arrayBufferToBase64(i16.buffer)}))}
      source.connect(proc);proc.connect(audioCtx.destination);setActive(true);setListening(true)
  ...
  return(<div style={bx}><div style={panelStyle}><div style={{fontSize:10,fontWeight:800,textTransform:'uppercase',letterSpacing:1.5,color:'var(--t3)',marginBottom:10}}>{'\u{1F399}\u{FE0F}'} Voice Assistant</div><div style={{display:'flex',gap:4,marginBottom:12}}>{VOICE_LANGS.map(l=>(<button key={l.code} onClick={()=>{if(active)stop();setLang(l.code)}} style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',gap:4,padding:'6px 10px',borderRadius:8,border:lang===l.code?'1px solid rgba(22,163,74,.4)':'1px solid var(--bd)',background:lang===l.code?'rgba(22,163,74,.08)':'transparent',cursor:'pointer'}}><span style={{fontSize:14}}>{l.flag}</span><span style={{fontSize:10,fontWeight:700,color:lang===l.code?'#16a34a':'var(--t2)'}}>{l.name}</span></button>))}</div><div style={{display:'flex',justifyContent:'center',marginBottom:12}}><button onClick={toggle} style={{width:72,height:72,borderRadius:'50%',border:'none',cursor:'pointer',background:active?'linear-gradient(135deg,#dc2626,#b91c1c)':'linear-gradient(135deg,#16a34a,#15803d)',color:'white',fontSize:28,display:'flex',alignItems:'center',justifyContent:'center',boxShadow:active?'0 0 0 8px rgba(220,38,38,.15), 0 4px 20px rgba(220,38,38,.3)':'0 4px 20px rgba(22,163,74,.3)',transition:'.2s'}}>{active?'\u23F9':'\u{1F399}\u{FE0F}'}</button></div><div style={{textAlign:'center',fontSize:11,fontWeight:700,color:active?(speaking?'#dc2626':'#16a34a'):'var(--t2)',marginBottom:8}}>{active?(speaking?'Speaking...':'Listening \u2014 speak in '+langInfo.name):'Tap mic to start'}</div>{inTx&&<div style={{padding:8,background:'rgba(22,163,74,.08)',borderRadius:8,fontSize:10,color:'var(--tx)',marginBottom:4,border:'1px solid rgba(22,163,74,.2)'}}><span style={{fontSize:8,fontWeight:700,color:'#16a34a'}}>YOU:</span> {inTx}</div>}{outTx&&<div style={{padding:8,background:'rgba(8,145,178,.08)',borderRadius:8,fontSize:10,color:'var(--tx)',marginBottom:4,border:'1px solid rgba(8,145,178,.2)'}}><span style={{fontSize:8,fontWeight:700,color:'#0891b2'}}>AI:</span> {outTx}</div>}{err&&<div style={{padding:8,background:'rgba(220,38,38,.08)',borderRadius:8,fontSize:9,color:'#dc2626',border:'1px solid rgba(220,38,38,.2)'}}>{err}</div>}</div><button onClick={()=>{if(!open)setOpen(true);else{if(active)stop();setOpen(false)}}} style={{width:52,height:52,borderRadius:'50%',background:active?'linear-gradient(135deg,#dc2626,#b91c1c)':open?'linear-gradient(135deg,#374151,#1f2937)':'linear-gradient(135deg,#16a34a,#15803d)',border:'none',cursor:'pointer',boxShadow:active?'0 0 0 6px rgba(220,38,38,.15), 0 4px 16px rgba(220,38,38,.3)':'0 4px 16px rgba(22,163,74,.3)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:22,color:'#fff',transition:'.2s'}} title="Voice Assistant">{active?'\u{1F534}':open?'\u2715':'\u{1F399}\u{FE0F}'}</button></div>)
}
...
export default function App() {
  ...
  const [selectedRole, setSelectedRole] = useState('cs')
  const [nameInput, setNameInput] = useState('')
  const [distInput, setDistInput] = useState('')
  const [loginErr, setLoginErr] = useState(false)
  const [crop, setCropState] = useState('maize')
  ...
  const [curMod, setCurMod] = useState('seed')
  const [tick, setTick] = useState(0)

  const mapRef = useRef(null)
  const mapInstanceRef = useRef(null)
  const geoLyrRef = useRef(null)
  const markerLyrRef = useRef(null)

  function setCrop(c) { setCropState(c); window.__shambamap_crop = c }

  useEffect(() => { window.__shambamap_crop = crop }, [crop])
  useEffect(() => { document.body.classList.toggle('light', !dark) }, [dark])

  // Apply NDMA data on mount
  useEffect(() => {
    D.forEach(function(d) {
      var ndma = NDMA_PHASE[d.id]
      if (ndma && ndma.phase && ndma.phase !== 'N/A') { d.dr = ndma.code; d.ndma_phase = ndma.phase; d.ndma_trend = ndma.trend; d.ndma_note = ndma.note }
  ...
  function doRender() {
    ...
    const L = window.L
    if (geoLyrRef.current) map.removeLayer(geoLyrRef.current)
    if (markerLyrRef.current) map.removeLayer(markerLyrRef.current)
    mod = curMod
    calcAll()
    geoLyrRef.current = L.geoJSON(KENYA_GEO, {
      style: function(f) {
        var dd = D.find(function(x) { return x.id === f.properties.id })
        if (!dd) return { fillColor: '#222', weight: 0.5, color: '#222e42', fillOpacity: 0.15 }
        var isSel = sel && sel.id === dd.id
        return { fillColor: getCol(dd), weight: isSel ? 2.5 : 1, color: isSel ? '#e2e8f0' : '#222e42', fillOpacity: isSel ? 0.7 : 0.45 }
      },
      onEachFeature: function(f, layer) {
        var dd = D.find(function(x) { return x.id === f.properties.id })
        if (!dd) return
        layer.on('click', function() { handleSelDist(dd.id) })
        layer.on('mouseover', function(e) { this.setStyle({ weight: 2, color: '#8b9ab8', fillOpacity: 0.6 }); layer.bindTooltip('<b>' + dd.n + '</b> ' + getLabel(dd), { sticky: true }).openTooltip(e.latlng) })
        layer.on('mouseout', function() { geoLyrRef.current.resetStyle(this); layer.unbindTooltip() })
      }
    }).addTo(map)
    markerLyrRef.current = L.layerGroup()
    D.forEach(function(dd) {
      var f = KENYA_GEO.features.find(function(x) { return x.properties.id === dd.id })
      if (!f) return
      L.marker([f.properties.centroid_lat, f.properties.centroid_lng], {
        icon: L.divIcon({ className: '', html: '<div style="text-align:center;pointer-events:none"><div style="font-size:7px;color:#5a6a84;text-shadow:0 1px 2px #000">' + dd.n + '</div><div style="font-size:10px;font-weight:700;color:#fff;text-shadow:0 1px 3px #000">' + getLabel(dd) + '</div></div>', iconSize: [80, 20], iconAnchor: [40, 10] })
      }).addTo(markerLyrRef.current)
    })
    markerLyrRef.current.addTo(map)
    // Use full render functions
    setDashHTML(renderDash() || '')
    setLegendHTML(renderLegend() || '')
    setListHTML(renderList() || '')
    renderLayers()
    if(sel) { var h = renderDetail(); if(h) setRightHTML(h) }
  }

  function getDashHTML() {
    var c = crop, cn = {maize:'Maize',wheat:'Wheat',beans:'Beans',sorghum:'Sorghum'}[c]
    var avgHAR = Math.round(D.reduce(function(s,d){return s+d.srr[c]},0)/D.length)
    var critical = D.filter(function(d){return d.srr[c]<20}).length
    if(mod==='seed') return '<div class="dk"><div class="dv" style="color:'+srrCol(avgHAR)+'">'+avgHAR+'%</div><div class="dl">Avg '+cn+' SRR</div></div><div class="dk"><div class="dv" style="color:var(--rd)">'+critical+'</div><div class="dl">Critical &lt;20%</div></div><div class="dk"><div class="dv" style="color:var(--cy)">'+D.length+'</div><div class="dl">Counties</div></div>'
    return '<div class="dk"><div class="dv" style="color:var(--gn)">'+D.length+'</div><div class="dl">Counties</div></div><div class="dk"><div class="dv" style="color:var(--cy)">'+cn+'</div><div class="dl">Crop</div></div>'
  }

  function getLegendHTML() {
    var t={seed:'Seed Replacement Rate',ndvi:'NDVI Crop Health',weather:'Temperature',pest:'Pest Risk Level',fertilizer:'Soil Nutrient Status',market:'Price vs NCPB Ref',horti:'Dominant Horti Crop',credit:'AgriScore'}
    return '<div class="sh">'+(t[mod]||mod)+'</div>'
  }

  function getListHTML() {
    var sorted=[].concat(D).sort(function(a,b){return (b.srr[crop]||0)-(a.srr[crop]||0)})
  ...
  function handleSelDist(id) {
    sel = D.find(function(d){return d.id===id})
    if (!sel) return
    mod = curMod
    // Use the full renderDetail function for complete module panels
    var html = renderDetail()
    if(html) setRightHTML(html)
    setRightVisible(true)
    setTick(t => t+1)
    if (!LIVE_WEATHER[id]) fetchOpenMeteo(id)
    var f = KENYA_GEO.features.find(function(x){return x.properties.id===id})
    if (f && mapInstanceRef.current) mapInstanceRef.current.flyTo([f.properties.centroid_lat, f.properties.centroid_lng], 7, {duration:0.5})
  }

  useEffect(() => {
    window.__shambamap_selDist = handleSelDist;
    window.__shambamap_toggleLayer = function(el) {
      var layerMod = el.getAttribute('data-mod');
      if (layerMod) {
        document.querySelectorAll('.ly[data-mod]').forEach(function(ly) { ly.classList.remove('on') });
        el.classList.add('on');
        handleSetModule(layerMod);
      }
    };
    return () => { delete window.__shambamap_selDist; delete window.__shambamap_toggleLayer }
  })

  function handleSetModule(m) { mod = m; setCurMod(m); if(sel) handleSelDist(sel.id) }

  function handleAsk() {
    if (!aiInput.trim()) return
    setAiMessages(prev => [...prev, {who:'user',text:aiInput.trim()}])
    setAiInput('')
    setTimeout(() => {
  ...
  const MODULE_BTNS=[
    {id:'seed',name:'Seed'},{id:'ndvi',name:'NDVI'},{id:'weather',name:'Weather'},{id:'pest',name:'Pest'},
    {id:'fertilizer',name:'Fertilizer'},{id:'market',name:'Market'},{id:'horti',name:'Horti'},{id:'credit',name:'AgriScore'},
    {id:'ksc',name:'KSC'},{id:'sowing',name:'Sowing'},{id:'yieldloss',name:'Yield Loss'},{id:'earlywarning',name:'Early Warning'},
    {id:'monsoon',name:'Readiness'},{id:'dss',name:'Schemes'},{id:'seedqr',name:'Seed QR'},{id:'report',name:'Report'},
    {id:'league',name:'League'},{id:'cpi',name:'CPI'},{id:'hustler',name:'Hustler'},{id:'gfw',name:'Deforestation'},
    {id:'vci',name:'VCI'},{id:'spi',name:'SPI'},{id:'acid',name:'Soil Acid'},{id:'cropfit',name:'Crop Fit'},
    {id:'tea',name:'Tea'},{id:'coffee',name:'Coffee'},{id:'pestwatch',name:'Pest Watch'},{id:'dealers',name:'Dealers'},
    {id:'kiamis',name:'KIAMIS'},{id:'beta',name:'BETA'},{id:'foodsec',name:'Food Sec'},{id:'eudr',name:'EUDR'},
    {id:'dairy',name:'Dairy'},{id:'dvs',name:'DVS'},{id:'exports',name:'Exports'},{id:'coldchain',name:'Cold Chain'},
    {id:'mech',name:'Mech'},{id:'ncpb',name:'NCPB'},{id:'phyto',name:'Phyto'},{id:'dfz',name:'DFZ'},
    {id:'flagship',name:'Flagship'},{id:'smartcrop',name:'Smart Crop'},{id:'iovsoc',name:'I/O'},
    {id:'varrec',name:'Variety'},{id:'planrec',name:'Plan'},{id:'seedsys',name:'Seed Sys'},
    {id:'mkulima',name:'Mkulima'},{id:'voice',name:'Voice'},{id:'pestcam',name:'Pest Cam'},
    {id:'officer',name:'Officer'},{id:'kenat',name:'vs National'},{id:'benchmark',name:'Benchmark'}
  ]
  const allowedMods=currentUser?(ROLES[currentUser.role]?.modules||[]):Object.keys(ROLES).reduce((a,k)=>{ROLES[k].modules.forEach(m=>{if(!a.includes(m))a.push(m)});return a},[])
  ...
  if(!loggedIn){
    return(<div className="login-overlay">
      <div className="login-box">
        ...
        <div className="lb-roles" id="roleGrid">
          {[
            {role:'cs',icon:'👔',name:'Cabinet Secretary',desc:'Full national access'},
            {role:'ps_crops',icon:'🌾',name:'PS Agriculture',desc:'State Dept for Agriculture (Crops)'},
            {role:'ps_horti',icon:'☕',name:'AFA Dir. Horticulture',desc:'Tea, coffee, flowers, exports'},
            {role:'ps_livestock',icon:'🐄',name:'PS Livestock Dev',desc:'Dairy, beef, ANITRAC traceability'},
            {role:'dir_soil',icon:'🧪',name:'Dir. Crops Development',desc:'Soil, fertilizer, land resources'},
            {role:'dir_pp',icon:'🐛',name:'KEPHIS Director',desc:'Pest & plant health'},
            {role:'kalro',icon:'📊',name:'KALRO Director',desc:'Research & innovation'},
            {role:'ksc',icon:'🏭',name:'KSC Regional Mgr',desc:'Seed supply & distribution'},
            {role:'cec',icon:'🏢',name:'County CEC Agriculture',desc:'County-level oversight'},
            {role:'subcounty',icon:'🤝',name:'Sub-County Agri Officer',desc:'Sub-county + ward view'},
        ...
        <div className="lb-field">
          <label>Name</label>
          <input id="loginName" placeholder="Enter your name" value={nameInput} onChange={e=>setNameInput(e.target.value)} />
        </div>
        <div className="lb-field" id="distField">
          <label>County (for sub-county/ward officers)</label>
          <select id="loginDist" value={distInput} onChange={e=>setDistInput(e.target.value)}>
            <option value="">All counties (national level)</option>
            {D.map(d=><option key={d.id} value={d.id}>{d.n}</option>)}
          </select>
        </div>
  ...
  return(<>
    ...
    <div className="top">
      ...
      <div className="mtabs" style={{display:activePage==='map'?'flex':'none'}}>
        ...
        <button className={'mt'+(curMod==='exports'?' on':'')} data-m="exports" onClick={()=>handleSetModule('exports')}>📦 Export Performance</button>
        <button className={'mt'+(curMod==='coldchain'?' on':'')} data-m="coldchain" onClick={()=>handleSetModule('coldchain')}>❄️ Cold Chain</button>
        <button className={'mt'+(curMod==='mech'?' on':'')} data-m="mech" onClick={()=>handleSetModule('mech')}>🚜 Mechanization</button>
        <button className={'mt'+(curMod==='ncpb'?' on':'')} data-m="ncpb" onClick={()=>handleSetModule('ncpb')}>🌾 NCPB Reserves</button>
        <button className={'mt'+(curMod==='phyto'?' on':'')} data-m="phyto" onClick={()=>handleSetModule('phyto')}>🛂 Phytosanitary</button>
        <button className={'mt'+(curMod==='dfz'?' on':'')} data-m="dfz" onClick={()=>handleSetModule('dfz')}>🛡️ Disease-Free Zones</button>
        ...
        <button className={'mt'+(curMod==='hustler'?' on':'')} data-m="hustler" onClick={()=>handleSetModule('hustler')}>💸 Hustler Fund</button>
        <button className={'mt'+(curMod==='gfw'?' on':'')} data-m="gfw" onClick={()=>handleSetModule('gfw')}>🌳 Deforestation</button>
        <button className={'mt'+(curMod==='acid'?' on':'')} data-m="acid" onClick={()=>handleSetModule('acid')}>⚗️ Soil Acidity Monitor</button>
        <button className={'mt'+(curMod==='cropfit'?' on':'')} data-m="cropfit" onClick={()=>handleSetModule('cropfit')}>🎯 Crop Fit Intelligence</button>
        <button className={'mt'+(curMod==='vci'?' on':'')} data-m="vci" onClick={()=>handleSetModule('vci')}>🛰️ VCI Drought (dekadal)</button>
        <button className={'mt'+(curMod==='spi'?' on':'')} data-m="spi" onClick={()=>handleSetModule('spi')}>💧 SPI Rainfall (monthly)</button>
        <button className={'mt'+(curMod==='tea'?' on':'')} data-m="tea" onClick={()=>handleSetModule('tea')}>🍵 Tea Auction (weekly)</button>
        <button className={'mt'+(curMod==='coffee'?' on':'')} data-m="coffee" onClick={()=>handleSetModule('coffee')}>☕ Coffee Auction (weekly)</button>
        <button className={'mt'+(curMod==='pestwatch'?' on':'')} data-m="pestwatch" onClick={()=>handleSetModule('pestwatch')}>🐛 Pest Watch (weekly)</button>
        <button className={'mt'+(curMod==='ndvi'?' on':'')} data-m="ndvi" onClick={()=>handleSetModule('ndvi')}>🛰 NDVI</button>
        <button className={'mt'+(curMod==='weather'?' on':'')} data-m="weather" onClick={()=>handleSetModule('weather')}>🌤 Weather</button>
        <button className={'mt'+(curMod==='pest'?' on':'')} data-m="pest" onClick={()=>handleSetModule('pest')}>🐛 Pest Alert</button>
        <button className={'mt'+(curMod==='fertilizer'?' on':'')} data-m="fertilizer" onClick={()=>handleSetModule('fertilizer')}>🧪 Fertilizer</button>
        <button className={'mt'+(curMod==='market'?' on':'')} data-m="market" onClick={()=>handleSetModule('market')}>📈 Market</button>
        <button className={'mt'+(curMod==='horti'?' on':'')} data-m="horti" onClick={()=>handleSetModule('horti')}>☕ Horticulture</button>
        <button className={'mt'+(curMod==='credit'?' on':'')} data-m="credit" onClick={()=>handleSetModule('credit')}>💳 AgriScore</button>
        <button className={'mt'+(curMod==='officer'?' on':'')} data-m="officer" onClick={()=>handleSetModule('officer')}>🤝 Officer</button>
        <button className={'mt'+(curMod==='ksc'?' on':'')} data-m="ksc" onClick={()=>handleSetModule('ksc')}>🏭 KSC Tracker</button>
        <button className={'mt'+(curMod==='sowing'?' on':'')} data-m="sowing" onClick={()=>handleSetModule('sowing')}>🌱 Sowing Progress</button>
        <button className={'mt'+(curMod==='yieldloss'?' on':'')} data-m="yieldloss" onClick={()=>handleSetModule('yieldloss')}>💸 Yield Loss</button>
        <button className={'mt'+(curMod==='earlywarning'?' on':'')} data-m="earlywarning" onClick={()=>handleSetModule('earlywarning')}>🚨 Early Warning</button>
        <button className={'mt'+(curMod==='benchmark'?' on':'')} data-m="benchmark" onClick={()=>handleSetModule('benchmark')}>📊 Benchmarking</button>
        ...
        <button className={'mt'+(curMod==='beta'?' on':'')} data-m="beta" onClick={()=>handleSetModule('beta')}>🏗 BETA Delivery</button>
        <button className={'mt'+(curMod==='foodsec'?' on':'')} data-m="foodsec" onClick={()=>handleSetModule('foodsec')}>🌾 Food Security</button>
        <button className={'mt'+(curMod==='eudr'?' on':'')} data-m="eudr" onClick={()=>handleSetModule('eudr')}>☕ EUDR Coffee</button>
        <button className={'mt'+(curMod==='dairy'?' on':'')} data-m="dairy" onClick={()=>handleSetModule('dairy')}>🥛 Dairy & Livestock</button>
        <button className={'mt'+(curMod==='league'?' on':'')} data-m="league" onClick={()=>handleSetModule('league')}>🏆 National League</button>
        <button className={'mt'+(curMod==='kenat'?' on':'')} data-m="kenat" onClick={()=>handleSetModule('kenat')}>🇰🇪 County vs National</button>
      </div>
      <div className="tright">
        <button onClick={()=>setSettingsOpen(true)} style={{height:'26px',padding:'0 8px',font:'inherit',fontSize:'9px',background:'var(--s2)',color:'var(--t2)',border:'1px solid var(--bd)',borderRadius:'4px',cursor:'pointer'}} title="API Settings">⚙ APIs</button>
        <button className="theme-toggle" id="themeBtn" onClick={()=>setDark(!dark)} title="Switch theme">{dark?'☀️ Light':'🌙 Dark'}</button>
        <select className="tsl" id="sCrop" value={crop} onChange={e=>setCrop(e.target.value)}>
          <option value="maize">Maize</option><option value="wheat">Wheat</option><option value="beans">Beans</option><option value="sorghum">Sorghum</option>
        </select>
        <select className="tsl" id="sSeason" value={season} onChange={e=>setSeason(e.target.value)}>
          <option value="lr26">Long Rains 2026</option><option value="sr25">Short Rains 2025</option><option value="lr25">Long Rains 2025</option>
        </select>
        <div className="srch"><em>🔍</em><input id="sinp" placeholder="Search county..." autoComplete="off" value={searchVal} onChange={e=>{setSearchVal(e.target.value);const q=e.target.value.toLowerCase().trim();if(q){const m=D.filter(d=>d.n.toLowerCase().indexOf(q)>=0);if(m.length===1)handleSelDist(m[0].id)}}} /></div>
        <div className="user-pill" id="userPill" onClick={()=>{setLoggedIn(false);setCurrentUser(null);sel=null}}>
          <div className="ua" id="userAvatar">{currentUser?.abbr||'CS'}</div>
          <div><div className="un" id="userName">{currentUser?.name||'Demo User'}</div><div className="ur" id="userRole">{currentUser?.roleName||'Cabinet Secretary'}</div></div>
        </div>
      </div>
    </div>
    ...
    <div className="wrap" style={{display:activePage==='map'?'flex':'none'}}>
      <div className="sl">
        ...
        <div className="dls" id="distList" dangerouslySetInnerHTML={{__html:listHTML}}></div>
      </div>
      <div className="mc">
        <div className="dash" id="dash" dangerouslySetInnerHTML={{__html:dashHTML}}></div>
        <div id="map" ref={mapRef}></div>
        <div className="lgd" id="legend" dangerouslySetInnerHTML={{__html:legendHTML}}></div>
        <div id="aiBtn" onClick={()=>setAiOpen(!aiOpen)} style={{position:'absolute',bottom:'16px',right:'16px',zIndex:800,width:'44px',height:'44px',borderRadius:'50%',background:'linear-gradient(135deg,#10b981,#059669)',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',boxShadow:'0 4px 16px rgba(16,185,129,.4)',fontSize:'20px',transition:'.2s'}} title="AI Assistant">🤖</div>
        {aiOpen&&<div id="aiPanel" style={{position:'absolute',bottom:'70px',right:'16px',zIndex:800,width:'320px',maxHeight:'400px',background:'var(--s1)',border:'1px solid var(--bd)',borderRadius:'12px',boxShadow:'0 8px 32px rgba(0,0,0,.5)',overflow:'hidden'}}>
          ...
          <div id="aiMessages" style={{height:'260px',overflowY:'auto',padding:'8px',fontSize:'11px'}}>
            {aiMessages.length===0&&<div style={{padding:8,background:'var(--s2)',borderRadius:8,fontSize:10,color:'var(--tx)'}}>Karibu! Ask about any county, crop, or module.</div>}
            {aiMessages.map((m,i)=>(<div key={i} style={{padding:6,background:m.who==='bot'?'var(--s2)':'rgba(16,185,129,.15)',borderRadius:6,marginBottom:4,fontSize:10,color:'var(--tx)',whiteSpace:'pre-wrap'}}>{m.text}</div>))}
          </div>
          <div style={{padding:'6px',borderTop:'1px solid var(--bd)',display:'flex',gap:'4px'}}>
            <input id="aiInput" placeholder="Uliza... e.g. Why is Kakamega hybrid adoption low?" value={aiInput} onChange={e=>setAiInput(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')handleAsk()}} style={{flex:1,height:'32px',padding:'0 10px',background:'var(--s2)',border:'1px solid var(--bd)',borderRadius:'6px',color:'var(--tx)',font:'inherit',fontSize:'10px',outline:'none'}} />
            <button onClick={handleAsk} style={{width:'32px',height:'32px',borderRadius:'6px',background:'var(--gn)',border:'none',color:'#fff',cursor:'pointer',fontSize:'12px'}}>→</button>
          </div>
        </div>}
      </div>
      <div className="sr" id="right">
        {rightVisible
          ? <div dangerouslySetInnerHTML={{__html:rightHTML}}></div>
          : <div className="empty"><div className="ei">🗺️</div><h3>Select a county</h3><p>Click map or list to see full agriculture intelligence — seed, NDVI, weather, pest, fertilizer, market, credit score.</p></div>
        }
      </div>
    </div>

    {/* SETTINGS OVERLAY */}
    {settingsOpen&&<div id="settingsOverlay" style={{display:'flex',position:'fixed',top:0,left:0,right:0,bottom:0,zIndex:9000,background:'rgba(0,0,0,.5)',backdropFilter:'blur(8px)',alignItems:'center',justifyContent:'center'}} onClick={()=>setSettingsOpen(false)}>
      <div style={{width:'440px',maxWidth:'95vw',background:'var(--s1)',border:'1px solid var(--bd)',borderRadius:'16px',padding:'28px',boxShadow:'0 20px 60px rgba(0,0,0,.5)'}} onClick={e=>e.stopPropagation()}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'20px'}}>
          <h2 style={{fontSize:'18px',fontWeight:800}}>⚙ Live API Settings</h2>
          <span onClick={()=>setSettingsOpen(false)} style={{cursor:'pointer',color:'var(--t3)',fontSize:'18px'}}>✕</span>
        </div>
        <div style={{padding:'12px',background:'rgba(16,185,129,.06)',border:'1px solid rgba(16,185,129,.2)',borderRadius:'8px',marginBottom:'12px'}}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
            <div style={{fontSize:'13px',fontWeight:700}}>🌤 Open-Meteo Weather</div>
            <span style={{fontSize:'9px',fontWeight:800,padding:'2px 8px',borderRadius:'3px',color:'#10b981',background:'rgba(16,185,129,.15)'}}>🟢 ACTIVE — No key needed</span>
          </div>
          <div style={{fontSize:'10px',color:'var(--t3)',marginTop:'5px'}}>Free, unlimited, no registration. Real temperature, humidity, rainfall & 7-day forecast for every Kenyan county. Backed by KMD reference.</div>
        </div>
        ...
        <div style={{padding:'12px',background:'rgba(245,158,11,.06)',border:'1px solid rgba(245,158,11,.2)',borderRadius:'8px',marginBottom:'12px'}}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
            <div style={{fontSize:'13px',fontWeight:700}}>🌵 NDMA Drought Phase</div>
            <span style={{fontSize:'9px',fontWeight:800,padding:'2px 8px',borderRadius:'3px',color:'#10b981',background:'rgba(16,185,129,.15)'}}>🟢 ASAL counties wired</span>
          </div>
          <div style={{fontSize:'10px',color:'var(--t3)',marginTop:'5px'}}>Official phase classification from NDMA monthly bulletins (Mar 2026): Normal / Recovery / Alert / Alarm.</div>
        </div>
        <div style={{padding:'12px',background:'rgba(168,85,247,.06)',border:'1px solid rgba(168,85,247,.2)',borderRadius:'8px',marginBottom:'12px'}}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
            <div style={{fontSize:'13px',fontWeight:700}}>🌱 KEPHIS Variety Catalogue</div>
            <span style={{fontSize:'9px',fontWeight:800,padding:'2px 8px',borderRadius:'3px',color:'#10b981',background:'rgba(16,185,129,.15)'}}>🟢 39 cultivars wired</span>
          </div>
          <div style={{fontSize:'10px',color:'var(--t3)',marginTop:'5px'}}>Maize, wheat, beans, sorghum, coffee & tea cultivars from KEPHIS National Variety List 2024 with breeder, year, AEZ, yield potential, traits.</div>
        </div>
        <div style={{padding:'10px',background:'var(--s2)',borderRadius:'6px',marginBottom:'16px',fontSize:'9px',color:'var(--t3)'}}>
          🔮 <b>Coming soon:</b> Sentinel-2 NDVI (Google Earth Engine), KMD station rainfall, KEPHIS seed registry, NDMA drought bulletins — all free tier.
        </div>
        <button onClick={()=>setSettingsOpen(false)} style={{width:'100%',height:'40px',background:'var(--gn)',color:'#fff',border:'none',borderRadius:'8px',font:'inherit',fontSize:'13px',fontWeight:700,cursor:'pointer'}}>Close</button>
      </div>
    </div>}

    <VoiceButton externalOpen={voiceOpen} setExternalOpen={setVoiceOpen} />
  </>)
}
